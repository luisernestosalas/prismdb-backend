...
app.post("/webhook/twilio", express.urlencoded({ extended: false }), (req, res) => {
  // ✅ FIX 422: Responder a Twilio INMEDIATAMENTE (< 15s requerido)
  // El procesamiento IA se hace en background (fire-and-forget)
  ...
  // Procesamiento async en background — no bloquea la respuesta a Twilio
  (async () => {
    try {
      ...
      // 2. Claude decide qué agente actúa Y detecta el stage del pipeline
      const decision = await claudeChat(
        `Router del Revenue OS. Analiza el mensaje y el historial. Responde SOLO JSON:
{"agent":"sdr|revenue|talent|none","reason":"una frase","stage":"contactado|interesado|demo_agendada|negociacion|cierre","stage_reason":"por qué este stage"}

Reglas de stage:
- contactado: primer contacto sin respuesta aún
- interesado: expresó interés, quiere saber más, respondió positivamente
- demo_agendada: confirmó una reunión, demo o llamada con fecha/hora
- negociacion: pidió propuesta, precio, comparó con competencia, negocia condiciones
- cierre: confirmó compra, firmó, pagó, dijo que sí al deal`,
        `Mensaje entrante: "${Body}"
Historial: ${JSON.stringify(memory)}`
      );
      const parsed = JSON.parse(decision.replace(/```json|```/g, "").trim());
      const agent = parsed.agent || "sdr";
      const newStage = parsed.stage || memory.pipeline_stage || "contactado";
...
Devuelve SOLO el mensaje a enviar, sin JSON.`
        ...
        // 4. Enviar respuesta por WhatsApp (usando sendWhatsApp, NO TwiML)
        if (agentResponse.trim()) {
          await sendWhatsApp(From, agentResponse.trim()).catch(console.error);
        }

        // 5. Guardar en memoria + actualizar stage del pipeline
        const prevStage = memory.pipeline_stage || "contactado";
        const newMemory = {
          ...memory,
          last_message: Body,
          last_agent: agent,
        ...
        if (db && memoryMode === "postgresql") {
          ...
          // Update pipeline table if stage changed
          if (prevStage !== newStage) {
            await db.query(`
              UPDATE pipeline SET stage = $1, updated_at = NOW()
              WHERE entity_id = $2
            `, [newStage, entityId]).catch(console.error);
          }
        } else {
          memoryStore[entityId] = newMemory;
          // Update in-memory pipeline
          if (prevStage !== newStage) {
            for (const [stage, leads] of Object.entries(crmPipeline)) {
              const idx = leads.findIndex(l => l.entity_id === entityId || l.phone === From);
              if (idx !== -1) {
                const lead = leads.splice(idx, 1)[0];
                lead.stage = newStage;
                (crmPipeline[newStage] ||= []).push(lead);
                break;
              }
            }
          }
        ...
        // Log the stage change as a separate event for the frontend to pick up
        if (prevStage !== newStage) {
          await logEvent("pipeline_stage_change", entityId, {
            from: prevStage,
            to: newStage,
            reason: parsed.stage_reason || "",
            message: Body,
            agent,
          });
          console.log(`[PIPELINE] ${entityId}: ${prevStage} → ${newStage}`);
        }

        await logEvent("whatsapp_handled", entityId, { message: Body, agent, response: agentResponse, stage: newStage });
      }
    } catch (e) {
